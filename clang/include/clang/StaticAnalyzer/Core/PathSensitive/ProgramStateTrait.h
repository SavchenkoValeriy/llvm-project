...
namespace ento {
  ...
  /// ImmutableSmallMap, ImmutableSmallSet, ImmutableList. The macro should not be used
  /// inside namespaces.
  ...
  /// can deal with.
  #define CLANG_ENTO_PROGRAMSTATE_MAP(Key, Value) llvm::ImmutableSmallMap<Key, Value>

  /// Declares an immutable map of type \p NameTy, suitable for placement into
  /// the ProgramState. This is implementing using llvm::ImmutableSmallMap.
  ///
  /// \code
  ...
  #define REGISTER_MAP_FACTORY_WITH_PROGRAMSTATE(Name, Key, Value) \
    using Name = llvm::ImmutableSmallMap<Key, Value>; \
    REGISTER_FACTORY_WITH_PROGRAMSTATE(Name)


  /// Declares an immutable set of type \p NameTy, suitable for placement into
  /// the ProgramState. This is implementing using llvm::ImmutableSmallSet.
  ///
  /// \code
  ...
  #define REGISTER_SET_WITH_PROGRAMSTATE(Name, Elem) \
    REGISTER_TRAIT_WITH_PROGRAMSTATE(Name, llvm::ImmutableSmallSet<Elem>)
  ...
  #define REGISTER_SET_FACTORY_WITH_PROGRAMSTATE(Name, Elem) \
    using Name = llvm::ImmutableSmallSet<Elem>; \
    REGISTER_FACTORY_WITH_PROGRAMSTATE(Name)
...
// Partial-specialization for ImmutableSmallMap.
...
// Partial-specialization for ImmutableSmallMap.
template <typename Key, typename Data, unsigned S, typename Info>
struct ProgramStatePartialTrait<llvm::ImmutableSmallMap<Key, Data, S, Info>> {
  using data_type = llvm::ImmutableSmallMap<Key, Data, S, Info>;
  using context_type = typename data_type::Factory &;
  using key_type = Key;
  using value_type = Data;
  using lookup_type = const value_type *;
  ...
  static data_type MakeData(void *const *p) {
    return p ? data_type((typename data_type::RawPtrTy) * p)
             : data_type((typename data_type::RawPtrTy) nullptr);
  }

  static void *MakeVoidPtr(data_type B) { return B.getRawPtr(); }
...
// Partial-specialization for ImmutableSmallSet.
template <typename Key, unsigned S, typename Info>
struct ProgramStatePartialTrait<llvm::ImmutableSmallSet<Key, S, Info>> {
  using data_type = llvm::ImmutableSmallSet<Key, S, Info>;
  using context_type = typename data_type::Factory &;
  using key_type = Key;
